...
#[receive(contract = "SavingBank", name = "insertAMount", payable)]
...
#[receive(contract = "SavingBank", name = "smashAmount")]
...
/// Get the balance of the piggy bank, only allowed by the owner.
#[receive(contract = "SavingBank", name = "balanceOf")]
fn piggy_balance_of<A: HasActions> (
    ctx: &impl HasReceiveContext,
    _state: &mut PiggyBankState,
) -> ReceiveResult<A> { 
    // Get the contract owner, i.e. the account who initialized the contract.
    let owner = ctx.owner();
    // Get the sender, who triggered this function, either a smart contract or
